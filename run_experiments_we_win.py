...
from experiment_setups.we_win_setup import we_win_setup
...
setup_list = [we_win_setup]
save_dir = "results_we_win"
...
def do_job(setup, n, rep_i):
    ...
    if verbose:
        print("")
        print("train zero percentage:", torch.mean((torch.norm(y_train, p=2, dim=1).float() <= 1e-10).float()))
        print("test zero percentage:", torch.mean((torch.norm(y_test, p=2, dim=1).float() <= 1e-10).float()))
